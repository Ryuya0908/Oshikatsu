...
// 表示制御: ログイン済みかつ参加者のみ閲覧可
if (!isset($_SESSION['username'])) {
    echo '<p>このコミュニティはログインしたユーザーのみ閲覧できます。<a href="login.php">ログイン</a> または <a href="sign-up.php">登録</a></p>';
    echo '<p><a href="community.php">コミュニティ一覧に戻る</a></p>';
    exit;
} else {
    $cm_check = $mysqli->prepare('SELECT id FROM community_members WHERE community_id = ? AND user_name = ?');
    $cm_check->bind_param('is', $community_id, $_SESSION['username']);
    $cm_check->execute();
    $cm_check->store_result();
    $is_member = $cm_check->num_rows > 0;
    $cm_check->close();

    if (!$is_member) {
        // 参加していないユーザーには参加ボタンのみ表示して閲覧不可にする
        echo '<p>このコミュニティは参加者のみ閲覧できます。</p>';
        echo '<form action="community.php" method="post">';
        echo '<input type="hidden" name="community_id" value="' . htmlspecialchars($community_id, ENT_QUOTES, 'UTF-8') . '">';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['post_content'])) {
    ...
    if ($user_name === null) {
        echo '<p>投稿するにはログインしてください。<a href="login.php">ログイン</a></p>';
    } else {
...
<body class="board-page">
    <div class="frame" style="max-width:800px; margin:40px auto;">
        <div class="logo" style="margin: 0 auto 32px auto; justify-content:center; align-items:center; gap:10px;">
            <div class="logo-icon">♥</div>
            <div class="logo-text">Oshikatu</div>
        </div>
        <div class="card" style="padding:32px 32px 32px 32px; background:#fff; box-shadow:0 4px 24px rgba(0,0,0,0.10),0 1.5px 4px rgba(0,0,0,0.08); border-radius:18px;">
        ...
        if ($posts && $posts->num_rows > 0) {
            while ($p = $posts->fetch_assoc()) {
                ...
                $comment_stmt->close();
                // コメント一覧を表示
                echo '<div class="comment-list" style="margin-left:2em;">';
                foreach ($comments_arr as $cm) {
                    echo '<div class="comment-row" style="margin-bottom:18px;padding-bottom:8px;border-bottom:1px solid #f0f0f0;">';
                    // 1行目：ユーザー名＋日付
                    echo '<div style="display:flex;align-items:center;gap:1em;">';
                    echo '<span class="comment-user" style="font-weight:bold;">' . htmlspecialchars($cm['user_name'], ENT_QUOTES, 'UTF-8') . '</span>';
                    echo '<span class="comment-date" style="font-size:0.9em;color:#888;">' . $cm['created_at'] . '</span>';
                    echo '</div>';
                    // 2行目：コメント本文
                    echo '<div class="comment-content" style="margin:4px 0 0 0;white-space:pre-wrap;word-break:break-word;">' . nl2br(htmlspecialchars($cm['comment_text'], ENT_QUOTES, 'UTF-8')) . '</div>';
                    // 3行目：返信ボタン（ダミー）
                    echo '<div style="margin:4px 0 0 1em;">';
                    echo '<button type="button" class="btn btn-outline" style="padding:2px 16px;font-size:0.95em;">返信</button>';
                    echo '</div>';
                    echo '</div>';
                }
                echo '</div>';
                // コメント投稿フォーム
                if (isset($_SESSION['username'])) {
                    echo '<form action="comment.php" method="post" style="margin:10px 0 10px 2em;max-width:600px;display:flex;align-items:flex-end;gap:8px;">';
                    echo '<input type="hidden" name="post_id" value="' . $p['id'] . '">';
                    echo '<textarea name="comment_text" rows="2" cols="40" required placeholder="コメントを書く" class="form-control" style="flex:1;padding:8px;border-radius:8px;border:1px solid #ccc;resize:vertical;"></textarea>';
                    echo '<button type="submit" class="btn btn-main" style="padding:8px 32px;">コメント送信</button>';
                    echo '</form>';
                }
                echo '</div>';
            }
        ...
        <?php if (isset($_SESSION['username'])): ?>
            ...
            <form action="board.php?id=<?php echo $community_id; ?>" method="post" style="margin-bottom:24px;">
                <div style="margin-bottom:12px;">
                    <label for="post_content">投稿内容:</label><br>
                    <textarea id="post_content" name="post_content" rows="6" cols="50" required placeholder="ここに投稿内容を入力" style="width:100%;max-width:700px;padding:10px;border-radius:8px;border:1.5px solid #bbb;resize:vertical;font-size:1em;"></textarea>
                </div>
                <button type="submit" class="btn btn-main" style="padding:10px 32px;font-size:1em;">投稿する</button>
            </form>
        <?php else: ?>
            <p>投稿するにはログインしてください。<a href="login.php">ログイン</a> または <a href="sign-up.php">新規登録</a></p>
        <?php endif; ?>
        </div>
